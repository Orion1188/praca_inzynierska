...
class QLearningSimulation:
    def __init__(self, num, acc_coeff=1, br_coeff=1, eps=1, v_max=40, road_length=1000, num_cars=30, 
                 time=20, reaction_time=1, v_disc=41, v_prev_disc=21, gap_disc=21, alpha=0.5, gamma=0.75):
        self.num = num
    ...
    def _qlearning_step(self, step):
        ...
        for i in range(n):
            ...
            state = [np.searchsorted(self.v_disc, self.v[i]),
            ...
            v_safe = self.v[(i+1) % n] + (gaps[i] - gap_des)/(self.reaction_time + full_stop_time)
            ...
            if v_des > self.v[i]:
                ...
                if self.q_table_accelerate[state[0], state[1], state[2]] >= self.q_table_steady[state[0], state[1], state[2]]:
                    action = 'accelerate'
                    state_new = [np.searchsorted(self.v_disc, v_des), state[1], state[2]]
                    v_new[i] = v_des
                else:
                    action = 'steady'
                    state_new = [np.searchsorted(self.v_disc, self.v[i]), state[1], state[2]]
                    v_new[i] = self.v[i]
        ...
        for i in range(n):
            if action == 'steady':
                ...
                self.q_table_steady[state] = (1 - self.alpha) * self.q_table_steady[state] + reward + self.gamma * self.q_table_steady[state_new]
            elif action == 'accelerate':
                reward = v_new[(i-1) % n] - self.history_v[step][(i-1) % n]
                self.q_table_accelerate[state] = (1 - self.alpha) * self.q_table_accelerate[state] + reward + self.gamma * self.q_table_accelerate[state_new]
...
if __name__ == '__main__':
    QLearningSimulation(4, time=300, alpha=0.8, gamma=0.8).simulation()
